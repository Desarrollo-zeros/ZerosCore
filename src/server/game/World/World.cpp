...
void World::SetInitialWorldSettings()
{
    ...
    ///- Seguridad del Core By Developer Fix.
	int clave;
	sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Verificando clave");
	scanf("%d, &clave");
	if (clave == 1234)
	{
		sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Clave ACEPTADA!");
	}
	else
	{
	    sLog->outError(LOG_FILTER_SERVER_LOADING, "ERROR: La clave del core no es correcta!. contactar: 0xc00007b@gmail.com");
		getch();
	    exit(1); 
	}
